...
fn ensure_pj_thread_registered() {
    thread_local! {
...
pub fn initialize_telephony(
    ...
    match (initResult) {
        Ok(_) => println!("Telephony Initialized Successfully"),
        Err(x) => return Err(x),
    };
    ...
    match (transportResult) {
        Ok(_) => println!("Transport Added Successfully"),
        Err(x) => return Err(x),
    };
    ...
    match (startResult) {
        Ok(_) => println!("Telephony Started Successfully"),
        Err(x) => return Err(x),
    };
    Ok(0)
}
...
pub fn init(
    loglevel: u32,
    incomming_call_behaviour: OnIncommingCall,
) -> Result<i8, TelephonyError> {
    ...
    if (status != pj::pj_constants__PJ_SUCCESS as i32) {
        ...
        return Err(TelephonyError::CreationError(
            "Could not Create Telephony Instance".to_string(),
        ));
    }
    ...
    if (status != pj::pj_constants__PJ_SUCCESS as i32) {
        ...
        return Err(TelephonyError::InitializationError(
...
pub fn start_telephony() -> Result<i8, TelephonyError> {
    ...
    if (status != pj::pj_constants__PJ_SUCCESS as i32) {
        ...
        return Err(TelephonyError::TelephonyStartError(
            "Could not Start Telephony".to_string(),
        ));
    };
    Ok(0)
}
    ...
    unsafe {
        ...
        acc_cfg_initialized.cred_info[0].data = data_pj_str_t;
            ...
            status = unsafe {
                pj::pjsua_acc_add(
                    acc_cfg.as_mut_ptr(),
                    pj::pj_constants__PJ_TRUE.try_into().unwrap(),
                    acc_id as *mut i32,
                )
            };
            ...
            if status != pj::pj_constants__PJ_SUCCESS as i32 {
                ...
                return Err(TelephonyError::AccountCreationError(
...
extern "C" fn on_incoming_call(
    acc_id: pj::pjsua_acc_id,
    call_id: pj::pjsua_call_id,
    rdata: *mut pj::pjsip_rx_data,
) {
    // let ci = unsafe {
    ...
    // };
    unsafe {
        pj::pjsua_call_answer(call_id, 200, std::ptr::null(), std::ptr::null());
    }
...
pub fn make_call(phone_number: &str, domain: &str) -> Result<i8, TelephonyError> {
    ...
    let call_extension_c_string_ok = match (call_extension_c_string) {
        Err(_y) => {
            return (Err(TelephonyError::CallCreationError(
    ...
    let call_extension_pj_str_t = pj::pj_str_t {
        slen: len,
    ...
    let make_call_restult = unsafe {
        pj::pjsua_call_make_call(
            ...
            opt,
            user_data_null,
            0 as *mut pj::pjsua_msg_data,
            0 as *mut pj::pjsua_call_id,
        )
    };
...
pub fn send_dtmf(digit: u32) -> Result<i8, TelephonyError> {
    ...
    let digits_pj_str_t = match (make_pj_str_t(digits)) {
        Err(_e) => {
            return Err(TelephonyError::DTMFError(
    ...
    if status != 0 {
        return Err(TelephonyError::DTMFError(
            "Cannot Send DTMF Tone".to_string(),
        ));
    }
    return Ok(0);
}
...
pub fn destroy_telephony() -> Result<i8, TelephonyError> {
    ...
    if status != 0 {
        return Err(TelephonyError::TelephonyDestroyError(
            "Error Occured during Telephony Destruction".to_string(),
        ));
    }
    Ok(0)
}
