...
pub fn ensure_pj_thread_registered() {
    ...
    REGISTERED.with(|reg| {
        if !reg.get() {
            ...
            unsafe {
                pj_sys::pj_thread_register(thread_name.as_ptr(), thread_desc.as_mut_ptr(), &mut thread);
            }
            reg.set(true);
        }
    });
...
pub fn account_setup(uri : String) -> Result<i32,PJSUAError> {
    ...
    let mut acc_cfg =  unsafe {
        let mut acc_cfg: Box<MaybeUninit<pj_sys::pjsua_acc_config>> = Box::new(MaybeUninit::uninit());
    ...
    acc_id = 0 ;
    status = unsafe {pj_sys::pjsua_acc_add(acc_cfg_ref, pj_sys::pj_constants__PJ_TRUE.try_into().unwrap(), acc_id as *mut i32 )} ;
...
extern "C" fn on_incoming_call(_acc_id: pj_sys::pjsua_acc_id, call_id: pj_sys::pjsua_call_id, _rdata: *mut pj_sys::pjsip_rx_data) {
    unsafe{ pj_sys::pjsua_call_answer(call_id, 200, std::ptr::null(), std::ptr::null()); } 
    println!("The accepted call id is: {}", call_id);
}

extern "C" fn on_incoming_call_ignore(_acc_id: pj_sys::pjsua_acc_id, call_id: pj_sys::pjsua_call_id, _rdata: *mut pj_sys::pjsip_rx_data) {
...
pub fn make_call(phone_number: &str, domain : &str) -> Result<i32,PJSUAError>{
    ...
    let call_extension_pj_str_t = pj_sys::pj_str_t { slen: len , ptr: call_extension_myptr };
